...
### New Features
- **Outline "Off" mode** — The outline panel now has three display modes: Always visible, Overlay, and Off. The toolbar button and **Ctrl+Shift+O** switch between Off and your chosen style, and unlike the old temporary hide, the Off state is remembered after you restart Bokuchi (see [Outline Panel](/guides/outline/))
- **Raspberry Pi (arm64 Linux) build** — A native arm64 `.deb` package is now available for Raspberry Pi OS (Bookworm). Bokuchi also detects a Raspberry Pi at startup and adjusts its renderer to avoid garbled display (see [System Requirements](/reference/system-requirements/))
